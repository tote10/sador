...
<div class="max-w-4xl mx-auto">
    ...
    <form action="{{ route('admin.partners.update', $partner->id) }}" method="POST" enctype="multipart/form-data" class="bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-xl space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $partner->name) }}" required class="block w-full px-4 py-3 bg-slate-950/60 border border-slate-800 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-sador-orange/50 focus:border-sador-orange text-xs transition duration-200">
            ...
            <div>
                ...
                <input type="text" name="role_label" id="role_label" value="{{ old('role_label', $partner->role_label) }}" class="block w-full px-4 py-3 bg-slate-950/60 border border-slate-800 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-sador-orange/50 focus:border-sador-orange text-xs transition duration-200">
        ...
        <div class="border-t border-slate-800 pt-6 flex items-center gap-6">
            @if ($partner->logo_url)
                <div class="w-16 h-16 rounded-2xl overflow-hidden border border-slate-800 bg-white p-2 shrink-0">
                    <img src="{{ $partner->logo_url }}" class="w-full h-full object-contain" alt="Partner logo current">
                </div>
            @endif
            <div class="flex-1">
